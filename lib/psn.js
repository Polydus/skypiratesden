...
function getAccessToken(authCode, callback) {
    ...
    if (refreshToken.length > 0) {
        request.post(psnURL.oauth, {
            form: { // Refreshing the token with refresh_token to avoid login again
            ...
            if (body == undefined) {
                ...
                if (!error) {
                    if ('access_token' in responseJSON && !('error' in responseJSON) && responseJSON != 'undefined') {
            ...
            if (!error) {
                if ('access_token' in responseJSON && !('error' in responseJSON)) {
                    ...
                    refreshInterval = setInterval(function () {
                        getAccessToken(accessToken, function () {
                            debug('access_token refreshed after ~10 minutes')
                        })
                    }, (responseJSON.expires_in - 60) * 150) // ~10 minutes 
